 ...
 */
export async function checkKillCriteria(debateConfidence) {
 ...
 */
export async function recordTradeResult(tradeId, pnl, _pnlPercent) {
    const db = getDatabase();
    ...
    if (pnl < 0) {
        ...
        if (riskState.consecutiveLosses === 3) {
            await alertKillCriteria('3 consecutive losses', 'Pause trading for 7 days', { consecutiveLosses: riskState.consecutiveLosses });
        }
    }
    else {
...
export async function updateDrawdown() {
    try {
        ...
        if (riskState.currentDrawdownPercent > 15) {
            await alertKillCriteria(`Drawdown ${riskState.currentDrawdownPercent.toFixed(1)}% > 15%`, 'Reduce position sizes 50%', { drawdownPercent: riskState.currentDrawdownPercent });
        }
    }
    catch (error) {
 ...
 */
async function logKillCriteriaEvent(trigger, action) {
    const db = getDatabase();
 ...
 */
export async function riskHealthCheck() {
    const issues = [];
    if (riskState.isPaused) {
