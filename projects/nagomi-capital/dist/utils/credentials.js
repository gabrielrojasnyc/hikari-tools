...
class CredentialLoader {
    ...
    get(name) {
        if (!this.loaded) {
            throw new CredentialError('Credentials not initialized. Call initialize() before using get()', 'CREDENTIALS_NOT_INITIALIZED');
        }
        const value = this.cache[name];
        if (value === undefined) {
            // Check if it's optional
            if (OPTIONAL_CREDENTIALS.includes(name)) {
                return '';
            }
            throw new CredentialError(`Required credential '${name}' not found in Vault`, 'CREDENTIAL_NOT_FOUND', name);
        }
        return value;
    }
    /**
    ...
    async loadAllCredentials() {
        ...
        if (errors.length > 0) {
            throw new CredentialError(`Vault credential loading failed:\n${errors.join('\n')}`, 'VAULT_LOAD_ERROR');
        }
        this.loaded = true;
     ...
     */
    async loadCredential(name, required) {
        ...
        try {
            ...
            if (mode !== 0o600) {
                throw new CredentialError(`Credential file '${name}' has incorrect permissions (${mode.toString(8)}). Expected 600.`, 'INCORRECT_PERMISSIONS', name);
            }
        }
        catch (error) {
        ...
        try {
            ...
            if (!content) {
                throw new CredentialError(`Credential '${name}' is empty`, 'EMPTY_CREDENTIAL', name);
            }
            ...
            if (placeholderPatterns.some(pattern => pattern.test(content))) {
                throw new CredentialError(`Credential '${name}' contains placeholder value`, 'PLACEHOLDER_CREDENTIAL', name);
            }
            ...
            if (error instanceof CredentialError)
                throw error;
            throw new CredentialError(`Failed to read credential '${name}'`, 'READ_ERROR', name);
        }
    }
    /**
...
export class CredentialError extends Error {
    ...
    credentialName;
    safeMessage;
    constructor(message, code, credentialName) {
...
export const credentials = {
     ...
     * Get a credential by name
     */
    get: (name) => credentialLoader.get(name),
    /**
     * Get an optional credential (returns empty string if not found)
     */
    getOptional: (name) => credentialLoader.getOptional(name),
    /**
