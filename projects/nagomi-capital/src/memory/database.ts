...
export class DatabaseManager {
  ...
  getUnprocessedSignals(limit: number = 10): any[] {
    const stmt = this.db.prepare(`
  ...
  saveDebate(debate: any): void {
    ...
    stmt.run(
      ...
      debate.signalId,
      debate.asset,
      debate.startTime.toISOString(),
      debate.endTime?.toISOString(),
      debate.bullScore,
      debate.bearScore,
      debate.flowScore,
      debate.decision,
      ...
      debate.positionSize,
      debate.thesis,
      JSON.stringify(debate.bullArgument),
      JSON.stringify(debate.bearArgument),
      JSON.stringify(debate.flowContext)
    );
  }
  ...
  saveRiskState(state: any): void {
    ...
    stmt.run(
      ...
      JSON.stringify(state.lastDebateConfidences || []),
      state.isPaused ? 1 : 0,
      state.pauseReason || null,
      state.pauseUntil?.toISOString() || null,
      state.trades30d,
      state.wins30d,
      state.winRate30d
    );
  ...
  logKillCriteriaEvent(event: { trigger: string; action: string; timestamp: string; state?: string }): void {
    const stmt = this.db.prepare(`
  ...
  updateTradeExit(asset: string, exitData: { exitPrice: number; exitTime: string }): void {
    const stmt = this.db.prepare(`
      UPDATE trades 
  ...
  closeTrade(tradeId: number, exitData: { exitPrice: number; exitTime: string; pnl: number }): void {
    const stmt = this.db.prepare(`
      UPDATE trades 
