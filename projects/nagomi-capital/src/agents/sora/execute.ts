...
import { getAlpacaClient, OrderRequest } from '../../ingestion/market/alpaca.js';
...
export async function executeTrade(decision: TradeDecision): Promise<ExecutionResult> {
  ...
  const sizing = calculateFullPosition(
    decision.conviction,
    decision.risk,
  ...
  try {
    ...
    if (result.success && result.orderId) {
      ...
      await alertTradeExecuted(
        decision.asset,
        decision.direction,
        result.filledQty || sizing.qty,
        result.filledPrice || decision.entry_price,
        result.orderId
      );
...
async function submitOrder(
  ...
  const { stopLossPrice, takeProfitPrice } = calculateBracketPrices(
    decision.entry_price,
    decision.direction,
    5, // 5% stop loss
    10  // 10% take profit
 ...
 */
function validateDecision(decision: TradeDecision): { valid: boolean; reason?: string } {
  // Check for NO_TRADE
  if (decision.direction === 'NO_TRADE') {
 ...
 */
export async function closePosition(symbol: string): Promise<ExecutionResult> {
  try {
    const alpaca = getAlpacaClient();
    const db = getDatabase();
 ...
 */
export async function getPositionsSummary(): Promise<{
  positions: Array<{
    symbol: string;
    qty: number;
    marketValue: number;
 ...
 */
export async function syncTradePnl(): Promise<void> {
  try {
    const alpaca = getAlpacaClient();
    const db = getDatabase();
