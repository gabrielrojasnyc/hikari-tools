...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case "query_database": {
        const { database_id, filter, sorts, page_size = 10 } = args as any;
      ...
      case "update_page": {
        const { page_id, properties } = args as any;
      ...
      case "create_page": {
        const { database_id, properties, content } = args as any;
